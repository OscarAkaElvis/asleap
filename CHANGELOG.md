...
Merged the fix from upstream PR #9 to resolve issue #8:

- Replaced deprecated DES `crypt()` and `setkey()` usage with the OpenSSL EVP API.
...
- Support for Airopeek NX files. Wireshark can be used to convert `.apc` files to libpcap format.
- Airjack integration for deauthentication.
- Windows support.
